...
const StudentsContent: React.FC<StudentsContentProps> = ({ 
  token,
  schoolId
}) => {
  ...
  const handleDeleteStudent = useCallback(async (id: string) => {
    ...
    try {
      const response = await fetch(`${baseApi}/school/delete-students/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
      ...
      if (response.ok) {
        ...
        const errorData = await response.json().catch(() => ({ message: 'Failed to remove student' }));
  ...
  const handleCreateCredentials = useCallback(async (student: Student) => {
    if (!token || isSubmitting) return;
  ...
  const confirmCreateCredentials = useCallback(async () => {
    if (!token || isSubmitting || !selectedStudent) return;

    setIsSubmitting(true);
    try {
      const response = await fetch(`${baseApi}/school/student-credentials`, {
        method: 'POST',
        headers: {
  ...
  }, [token, isSubmitting, selectedStudent, expiresInDays]);
  ...
  const handleCreateUser = useCallback(async (userData: {
    ...
    try {
      ...
      const requestBody = {
        ...
        ...(userData.role === "STUDENT" ? { classId: userData.classId } : {}),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">Students List</h2>
        {filteredStudents.length > 0 ? (
          <div className="space-y-4">
            {filteredStudents.map(student => (
              ...
              >
                <div>
                  <h3 className="text-md font-medium text-gray-800">{student.name}</h3>
                  <p className="text-sm text-gray-500">{student.email}</p>
                  {student.hasTemporaryCredentials && (
                    <div className="flex items-center mt-1">
                      <FaKey className="text-yellow-500 mr-1" size={12} />
                      <span className="text-xs text-yellow-600">
                        {student.credentialsExpired 
                          ? "Credentials expired" 
                          : `Temp access expires: ${new Date(student.credentialsExpireAt || "").toLocaleDateString()}`
                        }
                      </span>
                    </div>
                  )}
                ...
                <div className="flex space-x-3">
                  <button 
                    ...
                    aria-label={`Create credentials for ${student.name}`}
          ...
          <div className="py-8 text-center">
            ...
            <p className="text-sm text-gray-400 mt-1">
              {searchQuery ? 'Try adjusting your search query.' : 'Add a new student to get started.'}
            </p>
          </div>
        )}
      </div>
      ...
      {showCredentialsModal && selectedStudent && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
            ...
            <p className="mb-4">
              Create temporary access credentials for <span className="font-semibold">{selectedStudent.name}</span>
            </p>
            ...
            <div className="space-y-3 mb-4">
              <div>
                ...
                <div className="border p-2 rounded bg-gray-50 flex justify-between">
                  <span>{tempCredentials.studentName}</span>
                  <button 
                    onClick={() => copyToClipboard(tempCredentials.studentName)}
              ...
              <div>
                ...
                <div className="border p-2 rounded bg-gray-50 flex justify-between">
                  <span>{tempCredentials.studentEmail}</span>
                  <button 
                    onClick={() => copyToClipboard(tempCredentials.studentEmail)}
                    className="text-blue-600 hover:text-blue-800"
                  >
                    Copy
                  </button>
